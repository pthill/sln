...
public class OrdersController extends BaseController {
    ...
    private HttpJsonResultForAjax<OrderSuccessVO> commonSubmit(HttpServletRequest request,
        ...
        WebFrontSession.geDefSession(request,false).setAttribute("order_session", order_session);
        WebFrontSession.geDefSession(request,false).setAttribute("order_success_vo", orderSuccessVO);
    ...
    public String toPayfor(HttpServletRequest request, HttpServletResponse response,
                           ...
                           String paySessionstr,Integer type,String orderType) {
        ...
        if(orderType==null){
            //orderType为1表示是普通商品订单
            dataMap.put("orderType","1");
        }else{
            //orderType为2表示是积分商品订单
            dataMap.put("orderType",orderType.toString());
        }
    ...
    private String toOrderForActivity(HttpServletRequest request, ModelMap map, Integer productId,
        ...
        if (orderType != Orders.ORDER_TYPE_6) {
            ...
            ServiceResult<BigDecimal> transFeeRlt = sellerTransportService.calculateTransFee(
                productRlt.getResult(), goodRlt.getResult(), number, memberAddress.getCityId());
    ...
    public String jifen(@PathVariable String visitPath, HttpServletRequest request, ModelMap map,
        ...
        ServiceResult<List<MemberSpecialIntegral>> serviceResult=memberSpecialIntegralService.
                getMemberSpecialIntegralByMemberId(member.getId(),sellerId);
        if(!serviceResult.getSuccess()){
            if (ConstantsEJS.SERVICE_RESULT_CODE_SYSERROR.equals(serviceResult.getCode())) {
                throw new RuntimeException(serviceResult.getMessage());
            } else {

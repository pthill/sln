				...
				<div class="success-box clearfix">
						...
						<div class="p-item clearfix">
							<div class="p-img">
								<a href="${(domainUrlUtil.SLN_URL_RESOURCES)!}/product/${(product.id)!0}.html">
									<!--<#if product.source == 2>
									<img src="${(jdConfig.IMAGE_PATH_160)!}${(product.masterImg)!}" alt="" width="60" height="60">
									<#else>
									<img src="${(domainUrlUtil.SLN_IMAGE_RESOURCES)!}${(productGood.images)!}" alt="" width="60" height="60">
									</#if>
									-->
									<img src="${getImagePathMethod(product.source,product.productCode)}${(productGoods.images)!""}" alt="" width="60" height="60">
								</a>
							</div>
							<div class="p-info">
								<div class="p-name">
					...
					<div class="fr success-btns clearfix">
						<div class="success-btns-pdt">
							<a href="${(domainUrlUtil.SLN_URL_RESOURCES)!}/product/${(product.id)!0}.html" class="btn-tobback">详情</a>
							<a href="${(domainUrlUtil.SLN_URL_RESOURCES)!}/cart/detail.html?cartSource=1" class="btn-addtocart"><b></b>去购物车结算</a>
						</div>
					</div>
				</div>
			</div>
	...
	function addCart(productId,sellerId){
		...
		$.ajax({
			...
			success : function(data) {
				if(data.success){
					window.location.href=domain+"/cart/add.html?id=" + data.data+"&source=" + 1;
				}else{
					jAlert(data.message);
				}
			},

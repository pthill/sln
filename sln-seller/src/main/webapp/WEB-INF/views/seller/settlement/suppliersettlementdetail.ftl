...
currentBaseUrl="${domainUrlUtil.SLN_URL_RESOURCES}/supplier/settlement"/>
...
$(function() {
	...
	$("#back").click(function(){
 		window.location.href="${domainUrlUtil.SLN_URL_RESOURCES}/supplier/settlement";
	});
		...
		$.messager.confirm('确认', '确定对该结算账单提出质疑吗？', function(r) {
			if (r) {
				$.messager.progress({
		 		...
		 		$("#addForm").attr("action", "${domainUrlUtil.SLN_URL_RESOURCES}/supplier/settlement/checkDoubt")
	  				 .attr("method", "POST")
	  				 .submit();
		...
		$.messager.confirm('确认', '确定通过该结算账单吗？', function(r) {
			if (r) {
				$.messager.progress({
		 		...
		 		$("#addForm").attr("action", "${domainUrlUtil.SLN_URL_RESOURCES}/supplier/settlement/checkOver")
	  				 .attr("method", "POST")
	  				 .submit();
				$.messager.progress('close');
			}
	...
	$("#payOver").click(function(){
		$.messager.confirm('确认', '确定已经收到商家打款了吗？', function(r) {
			if (r) {
				$.messager.progress({
					text : "提交中..."
				});
		 		$("#addForm").attr("action", "${domainUrlUtil.SLN_URL_RESOURCES}/supplier/settlement/payOver")
	  				 .attr("method", "POST")
	  				 .submit();
				$.messager.progress('close');
			}
	...
	function onExpandRow(index,row){
        ...
        ddv.datagrid({
           ...
           singleSelect:true,
           method:'get',
           url:'${domainUrlUtil.SLN_URL_RESOURCES}/supplier/settlement/getSettlementOp?orderId='+row.id,
			loadMsg : '数据加载中...',
			height : 'auto',
			columns : [[{
				field : 'productName',
				...
				<div class="whtitdiv" data-options="region:'north'"
					...
					<div class="col-lg-12 col-sm-12 col-xs-12">
						...
						<@form.form method="post" class="form-horizontal" id="addForm" 
							...
							<!-- 结算状态：1、账单生成；2、商家审核通过；3、供应商核对通过；4、供应商核对质疑；5、对账完成；6、支付完成 -->
							<#if settlement.status == 2 || settlement.status == 4>
							<!-- 结算状态：2、商家审核通过；4、供应商核对质疑，此时可以输入质疑 -->
							<div class="form-group">
								<label class="col-lg-2 control-label">供应商质疑：</label>
								<div class="col-lg-4">
							...
							<!-- 结算状态：1、账单生成；3、供应商核对通过；5、对账完成；6、支付完成，此时不可以输入质疑 -->
							<div class="form-group">
								<label class="col-lg-2 control-label">供应商质疑：</label> 
								<div class="col-lg-4">
									<input type="text" disabled="disabled"
							...
							<div class="form-group">
								<p class="p12 p-item">
									<label class="col-lg-2 control-label">商家解释：</label> 
									<div class="col-lg-4">
										<input type="text" disabled="disabled"

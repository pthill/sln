			...
			<div id="bodyhaiheyungu" style="overflow-y: auto; overflow-x: hidden;">
				<div class="col-lg-12 col-sm-12 col-xs-12">
					...
					<form method="post" id="addform" class="form-horizontal"
						action="${domainUrlUtil.SLN_URL_RESOURCES}/seller/promotion/actintegral/update"
						enctype="multipart/form-data" data-bv-message="该项必填">
						 	<input type="hidden" id="id" name="id" value="${(actIntegral.id)!}" />
						 	<input type="hidden" id="productName"  name="productName" value="${(actIntegral.productName)!}" />
						...
						<div class="form-group">
							...
							<div class="col-lg-10">
								...
								<label id="productName1">${(actIntegral.productName)!}</label>
							</div>
						</div>
		...
		<div class="easyui-layout" data-options="fit:true">
			<table id="brandDataGrid" class="easyui-datagrid"
				data-options="
							...
							pagination:true,
							pageSize:'20',
							url:'${domainUrlUtil.SLN_URL_RESOURCES}/seller/product/list?q_state=6&q_isWelfareProduct=2',
							method:'get'">
				<thead>
					<tr>
...
<div style="display: none">
	...
	<script language="javascript">
		function productSubmit() {
			...
			$.ajax({
				url: "${domainUrlUtil.SLN_URL_RESOURCES}/seller/promotion/actintegral/getIntegralScale",
				type:'GET',
				success:function(data){
					if(data.success){
						$("#price").val(selectedRow[0].mallPcPrice * data.data.integralScale);
					}
				}
			});

										...
										<div class='goods-item goods-item-extra'>
											<div class='p-img'>
												<a href='${(domainUrlUtil.SLN_URL_RESOURCES)!}/jifen/${(actIntegral.id)!0}.html' target="_blank">
													<img width="80" height="80" src='${(domainUrlUtil.SLN_IMAGE_RESOURCES)!}${(productGoods.images)!(actIntegral.image)}'>
												</a>
											</div>
											<div class='goods-msg'>
												<div class='goods-msg-gel'>
													<div class='p-name'>
														<a href='${(domainUrlUtil.SLN_URL_RESOURCES)!}/jifen/${(actIntegral.id)!0}.html' target='_blank'>${product.name1!''}</a>
													</div>
													<div class='p-state'>
														<strong class=''>${productGoods.normName!''}</strong>
													</div>
													<div class='p-price'>
														<strong class=''>${(actIntegral.price)!0} 积分</strong>
													</div>
													<div class='p-num'>
										...
										<div class='cbox'>
											...
											<div id="orderBeanItem" class="inner">
													...
													<div class="cho-result" >
															...
															本次支付消耗积分：
															<font color="red">${(actIntegral.price * number)!'0'}</font>
															&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;
															可用积分：
															...
															<span>【${seller.sellerName}】专用:${(sellerSpecialIntegral)!''}</span>
															
															<#if (member.integral + sellerSpecialIntegral) < (actIntegral.price * number)>
																...
																兑换本订单积分需要：<font color="red">${((actIntegral.price * number - member.integral - sellerSpecialIntegral) / config.integralScale)?string('0.##')}</font>元
																</div>
															<#else>
															...
															<span class="beans-cho-tip">使用规`则(<i style="color:#edd28b;">积分满${config.integralScale!0}即可使用：每次使用积分为n*${config.integralScale!0}</i>)</span>
															<span class="clr"></span>
															-->
													</div>
										        </div>
			...
			<div class='statistic' style='float:right'>
				...
				<div class='list'>
					<span>
					 ...
					 <em class='price' id='discountAmountPriceId'>${(actIntegral.price * number)!'0'}</em>
				</div>
				<div class='list'>
					<span>应付积分 ：</span>
				...
				<div class='checkout-buttons'>
					<div class='sticky-wrap'>
						<div class='inner'>
							...
							<span class='total'>
								应付积分：
								<strong id='payPriceId'>${(actIntegral.price * number)!'0'}</strong>
							</span>
						</div>
					</div>
				</div>
					...
					if (result.success) {
						...
						var payAmount = data.payAmount;
						 ...
						 if (goJumpPayfor) {
							...
							newurl = successUrl + "?paySn=" + paySn + "&type=5" +

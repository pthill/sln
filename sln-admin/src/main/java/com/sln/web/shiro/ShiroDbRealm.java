...
public class ShiroDbRealm extends AuthorizingRealm {
    ...
    protected AuthorizationInfo doGetAuthorizationInfo(PrincipalCollection principals) {
        ...
        ServiceResult<List<SystemResources>> resourcesResult = systemResourcesRolesService
            .getResourceByRoleId(systemAdmin.getRoleId());
        ...
        ServiceResult<SystemRoles> roleResult = systemRolesService.getSystemRolesById(systemAdmin.getRoleId());
        if(!roleResult.getSuccess()){
            log.error(resourcesResult.getMessage());
        }
        if(roleResult.getResult().getRoleType().equals("1")){//当前登录的角色是平台
            info.addRole("admin");
        }else{//当前登录的是业务管理方的角色
            info.addRole("operation");
        }
        return info;
    }
}

...
<div class="wrapper">
    ...
    <#--1.addForm----------------->
        ...
        <@form.form method="post" class="validForm" id="addForm" name="addForm">
            <input type="hidden" id="id" name="id" value="${parkAdvantage.id}">
                ...
                <dd class="dd-group">
                    <div class="fluidbox">
                        <p class="p12 p-item">
                            ...
                            <input type="text" id="title" name="title" value="${(parkAdvantage.title)!''}"  class="txt w190 easyui-validatebox" missingMessage="标题必填，2-10个字符" data-options="required:true,validType:['code','length[2,10]']"/>
                        </p>
                    </div>
                    <div>
                        <p class="p12 p-item">
                            <label class="lab-item"><font class="red">*</font>排序: </label>
                            <input type="text" id="order" name="order" value="${(parkAdvantage.order)!''}" class="txt w190 easyui-numberbox" missingMessage="排序必填，1-5个字符" data-options="required:true,validType:'length[1,5]'"/>
                        </p>
                    </div>
                    <div class="fluidbox">
                        <p class="p12 p-item">
                            ...
                            <select name="parkId" id="parkId" class="txt w200"  data-options="required:true" >
                                ...
                                <#if parks??>
                                    <#list parks as park>
                                        <option <#if parkAdvantage.parkId??&&parkAdvantage.parkId==park.id>selected</#if> value="${park.id}">${park.parkName}</option>
                                    </#list>
                                </#if>
                            </select>
                        </p>

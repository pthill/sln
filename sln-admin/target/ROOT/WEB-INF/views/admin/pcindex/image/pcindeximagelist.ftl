...
<script language="javascript">
	...
	//为客户端装配本页面需要的字典数据,多个用逗号分隔
		...
		$("#a-gridAdd").click(function(){
	 		window.location.href="${(domainUrlUtil.SLN_URL_RESOURCES)!}/admin/pcindex/image/add";
		});
		...
		$("#a-gridEdit").click(function(){
			...
			if(!selected) {
	 		...
	 		window.location.href="${(domainUrlUtil.SLN_URL_RESOURCES)!}/admin/pcindex/image/edit?pcIndexImageId="+selected.id;
		});
		...
		$("#a-gridDel").click(function(){
            ...
            $.messager.confirm('提示', '确定删除吗？', function(r){
                if (r){
                    ...
                    $.ajax({
                        type:"POST",
                        url: "${domainUrlUtil.SLN_URL_RESOURCES}/admin/pcindex/image/delete",
                        dataType: "json",
		...
		$("#a-gridUp").click(function(){
	        ...
	        $.messager.confirm('提示', '确定启用吗？', function(r){
	            if (r){
	                ...
	                $.ajax({
	                    type:"POST",
	                    url: "${domainUrlUtil.SLN_URL_RESOURCES}/admin/pcindex/image/up",
	                    dataType: "json",
	                    data: "id="+selected.id,
	                    cache:false,
	                    success:function(data, textStatus){
		...
		$("#a-gridPre").click(function(){
	        ...
	        $.messager.confirm('提示', '确定预使用吗？', function(r){
	            if (r){
	                ...
	                $.ajax({
	                    type:"POST",
	                    url: "${domainUrlUtil.SLN_URL_RESOURCES}/admin/pcindex/image/pre",
	                    dataType: "json",
	                    data: "id="+selected.id,
	                    cache:false,
	                    success:function(data, textStatus){
	    ...
	    $("#a-gridDown").click(function(){
	        ...
	        $.messager.confirm('提示', '确定停用吗？', function(r){
	            if (r){
	                ...
	                $.ajax({
	                    type:"POST",
	                    url: "${domainUrlUtil.SLN_URL_RESOURCES}/admin/pcindex/image/down",
	                    dataType: "json",
	                    data: "id="+selected.id,
	                    cache:false,
	                    success:function(data, textStatus){
	...
	function addFormat(value, row, index){
		return "<a href='${domainUrlUtil.SLN_FRONT_URL}" + 
 	  		value + "' style='color:#276892' target='_blank'>点击访问</a>";
	}
...
<div data-options="region:'center'" border="false">
	<table id="dataGrid" class="easyui-datagrid"
			data-options="rownumbers:true
						...
						,pageSize:'${pageSize}'
						,fit:true
    					,url:'${(domainUrlUtil.SLN_URL_RESOURCES)!}/admin/pcindex/image/list'

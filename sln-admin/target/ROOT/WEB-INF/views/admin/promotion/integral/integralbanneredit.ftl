...
<link rel="stylesheet"
	href="${(domainUrlUtil.SLN_STATIC_RESOURCES)!}/resources/admin/jslib/jquery.boxer/css/jquery.fs.boxer.css"
	type="text/css">
...
$(function(){
    ...
    $("#back").click(function(){
 		window.location.href="${domainUrlUtil.SLN_URL_RESOURCES}/admin/actintegralbanner/banner";
	});
	$("#add").click(function(){
		if($("#addForm").form('validate')){
	 		$("#addForm").attr("action", "${domainUrlUtil.SLN_URL_RESOURCES}/admin/actintegralbanner/banner/update")
  				 .attr("method", "POST")
  				 .submit();
  		}
	});
	...
	<div class="formbox-a">
		<h2 class="h2-title">
			修改积分商城首页轮播图<span class="s-poar"><a class="a-back"
				href="${domainUrlUtil.SLN_URL_RESOURCES}/admin/actintegralbanner/banner">返回</a></span>
		</h2>
					...
					<div class="fluidbox">
						<p class="p12 p-item">
							...
							<#if (actIntegralBanner.image)??> &nbsp;&nbsp;&nbsp;&nbsp;<a
								href="${domainUrlUtil.SLN_IMAGE_RESOURCES}${(actIntegralBanner.image)!''}"

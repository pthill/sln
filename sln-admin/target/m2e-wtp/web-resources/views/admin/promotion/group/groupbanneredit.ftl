...
$(function(){
	...
	$("#back").click(function(){
 		window.location.href="${domainUrlUtil.SLN_URL_RESOURCES}/admin/actgroupbanner/banner";
	});
	$("#add").click(function(){
		if($("#addForm").form('validate')){
	 		$("#addForm").attr("action", "${domainUrlUtil.SLN_URL_RESOURCES}/admin/actgroupbanner/banner/update")
  				 .attr("method", "POST")
  				 .submit();
  		}
	});
...
<div class="wrapper">
	<div class="formbox-a">
		<h2 class="h2-title">修改团购首页轮播图<span class="s-poar"><a class="a-back" href="${domainUrlUtil.SLN_URL_RESOURCES}/admin/actgroupbanner/banner">返回</a></span></h2>
				...
				<dd class="dd-group">
					<div class="fluidbox">
						<p class="p12 p-item">
							...
							<input class="easyui-validatebox txt w280" type="text" id="title" name="title" value="${(actGroupBanner.title)!''}" data-options="required:true,validType:'length[1,50]'" >
						</p>
					</div>
					<br/>
					<div class="fluidbox">
					...
					<div class="fluidbox">
						<p class="p12 p-item">
							...
							<#if (actGroupBanner.image)??> 
									&nbsp;&nbsp;&nbsp;&nbsp;<a href="${domainUrlUtil.SLN_IMAGE_RESOURCES}${(actGroupBanner.image)!''}" class="colorbox">查看图片</a>
							</#if>
						</p>
						<p class="p12 p-item">
							<label class="lab-item">&nbsp;</label>
